...
export default function AdminWordsPage() {
  ...
  const handleDelete = (id: string) => {
    ...
    (async () => {
      try {
        const resp = await fetch(`/api/words/${encodeURIComponent(id)}`, { method: 'DELETE', credentials: 'same-origin' });
    ...
    (async () => {
      ...
      try {
        ...
        if (isEditing) {
          ...
          const resp = await fetch(`/api/words/${encodeURIComponent(id)}`, { method: 'PUT', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editing) });
          if (!resp.ok) throw new Error('Update failed');
          toast({ title: 'Updated', description: 'Item updated' });
        } else {
          const resp = await fetch('/api/words', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editing) });
          if (!resp.ok) throw new Error('Create failed');
          toast({ title: 'Created', description: 'Item created' });
        }
        const refreshed = await fetch('/api/words');
